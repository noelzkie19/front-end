...
interface Props {
	setCommunicationFeedback: (e: any) => void;
}
const CustomerCommunicationFeedback: React.FC<Props> = (Props) => {
	const {setCommunicationFeedback} = Props;
	const {
	...
	useEffect(() => {
		const casecommFeedback = customerCaseCommunicationFeedbackList.map((feedback: CustomerCaseCommunicationFeedbackModel) => {
			var item: CustomerCommunicationFeedBackTypeRequestModel = {
				...
				feedbackAnswerId: feedback.feedbackAnswerId,
				feedbackAnswer: feedback.feedbackAnswerName,
				communicationFeedbackDetails: feedback.communicationFeedbackDetails,
				communicationSolutionProvided: feedback.communicationSolutionProvided,
				createdBy: userId !== undefined ? parseInt(userId) : 0,
				updatedBy: 0,
			};
			return item;
		});
		setCommunicationFeedback(casecommFeedback);
	}, [customerCaseCommunicationFeedbackList]);
	...
	useEffect(() => {
		if (selectedFeedbackType !== undefined && selectedFeedbackType !== null) {
	...
	useEffect(() => {
		if (selectedFeedbackCategory !== undefined && selectedFeedbackCategory !== null) {
	...
	const AddFeedback = () => {
		...
		if (customerCaseCommunicationFeedbackList !== undefined) {
			...
			if (isError === false) {
				...
				const newDataToStore = storedData.concat(requestToValidate);
				setCustomerCaseCommunicationFeedbackList(newDataToStore);
...
export default CustomerCommunicationFeedback;
